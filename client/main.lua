...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        if Config.Stores[k].shopType == "clothing" then
            ...
            AddTextComponentString("Thay quần áo")
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Hớt tóc")
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Thay đồ')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Hớt tóc')
                            end
                            if IsControlJustPressed(0, Keys["E"]) then
                                if Config.Stores[k].shopType == "clothing" then
                                    ...
                                    ESX.TriggerServerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "character", label = "Quần áo", selected = true},
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                if dist < 15 then
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData ~= nil and PlayerData.job ~= nil and PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.3, '~g~E~w~ - Thay đổi trang phục')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    ESX.TriggerServerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Đồng phục", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                            {menu = "myOutfits", label = "Trang phục", selected = false, outfits = result},
                                            {menu = "character", label = "Quần áo", selected = false},
                                            {menu = "accessoires", label = "Phụ kiện", selected = false}
                                        })
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    ESX.TriggerServerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "Trang phục", selected = true, outfits = result},
        })
    end)
end)
...
function GetMaxValues2()
    ...
    for k, v in pairs(clothingCategorys) do
        if v.type == "variation" then
            if k == "t-shirt" then
                print(GetNumberOfPedDrawableVariations(ped, v.id))
            end
            maxModelValues[k].item = GetNumberOfPedDrawableVariations(ped, v.id)
            maxModelValues[k].texture = GetNumberOfPedTextureVariations(ped, v.id, GetPedDrawableVariation(ped, v.id)) -1
        end
...
function openMenu(allowedMenus)
    ...
    TriggerEvent('skinchanger:getSkin', function(esxSkin)
        
        --local trackerMeta = PlayerData.metadata["tracker"]

        skin = Config.ConvertFromESXToQbus(skinData, esxSkin)
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent("ESX:Notify", "Bạn không thể tháo vòng tay", "success", 8000)
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerEvent('skinchanger:getSkin', function(esxSkin)
        ...
        skin = Config.ConvertFromQbusToESX(skinData, esxSkin)
        TriggerServerEvent('esx_clotheshop:saveOutfit', data.outfitName, skin)
    end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    TriggerEvent("ESX:Notify", "Bạn đã xoá trang phục"..data.outfitName.." !", "error", 8000)
end)
...
function SaveSkin()
    ...
    TriggerEvent('skinchanger:getSkin', function(esxSkin)
        -- TriggerServerEvent('esx_skin:save', skin)
        -- TriggerServerEvent("qb-clothing:saveSkin", model, clothing, esxSkin)
        skin = Config.ConvertFromQbusToESX(skinData, esxSkin)
        TriggerServerEvent('esx_skin:save', skin)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if oData.outfitName ~= nil then
        closeMenu()
        TriggerEvent("ESX:Notify", "Đã mặc trang phục :"..oData.outfitName..'!', "success", 8000)
    end
end)
